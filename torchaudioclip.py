...
def stft_and_db(s, n_fft=512) -> Tensor:
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    # print(s)
    if not isinstance(s, torch.Tensor):
        s = torch.tensor(s, device=device, dtype=torch.float32)
    # print(s.shape)
    # print(s.stft(256, normalized=False).shape)
    return amplitude_to_DB(magphase(
            s.stft(n_fft, normalized=False)
        )[0],
        multiplier=10.,
        amin=1e-05,
        db_multiplier=1e-05)/torch.tensor(80., device=device)
                     # , dtype=torch.float16, device=device)  # auto flatten by DNN